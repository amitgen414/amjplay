...
const brickBlast = (function () {
  ...
  const exitGame = () => window.location.replace("/");
  ...
  ns.init = () => {
    ...
    document.addEventListener("keyup", (e) => e.key === "Escape" && exitGame());
  ...
  ns.drawNotifications = (messages = [], color = "#ffffff", y = cv.height / 2, lineheight = 40) => {
    let relativeY = y;
    messages.forEach(message => {
      ctx.font = `25px ${font}`;
      ctx.fillStyle = color;
      const measure = ctx.measureText(message);
      ctx.fillText(message, (cv.width - measure.width) / 2, relativeY);
      ctx.fillText(message, (cv.width - measure.width) / 2, relativeY);
      relativeY += lineheight;
    });
  }
  ...
  ns.move = () => {
    ...
    if (ballY <= canvasOffset) {
      ...
      ns.drawNotifications([
        "You lost! Press a key to restart",
        "Press escape to exit"
      ], "#ff0000");
  ...
  ns.checkGameOver = () => {
    if (levelScore === 3000) {
      ns.drawNotifications([
        "Congrats! you have won this level.",
